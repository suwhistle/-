...
LED led;		 // 板载LED
Buzzer buzzer;   // 蜂鸣器
SnakeGame snakeGame(&screen.spr, &keys);
...
void snake() { 
    ...
    while (snakeGame.isRunning()) {
        ...
        // 将 Sprite 内容推送到屏幕 (这一步很关键，否则屏幕不显示)
        // 假设你是横屏 240x170 或类似分辨率，根据实际情况调整
        // 注意：SnakeGame 内部只在 Sprite 上画图，这里负责推送到 LCD
    ...
    // 3. 游戏退出后的清理
    Serial.println("退出贪吃蛇游戏");
    
    // 恢复屏幕设置 (字体、对齐等，防止影响主菜单)
    screen.spr.fillSprite(TFT_BLACK);
